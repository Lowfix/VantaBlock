...
export function DeployServerModal({ open, onClose, onDeployed }: DeployServerModalProps) {
  ...
  async function handleDeploy() {
    ...
    setDeploying(true);
    try {
      const res = await demoFetch("/api/servers", {
        method: "POST",
        credentials: "include",
  ...
  return (
    <Modal
      ...
      description="Choose a type, version, and plan for your new Minecraft server."
      className="!max-w-xl"
    >
        ...
        <div className="flex items-center justify-between rounded-lg border border-line bg-panel-2 px-4 py-3">
          <div>
            ...
            <p className="mt-0.5 text-xs text-text-lo">
              Automatically create a free {name.trim() ? `${name.trim().toLowerCase().replace(/\s+/g, "-")}` : "yourserver"}.vantablock.net address.
            </p>
          </div>
