...
export function DatabaseTab({ identifier }: { identifier: string }) {
  ...
  const load = useCallback(async () => {
    try {
      const res = await demoFetch(`/api/servers/${identifier}/databases`, { credentials: "include" });
  ...
  async function handleCreate() {
    ...
    try {
      const res = await demoFetch(`/api/servers/${identifier}/databases`, {
        method: "POST",
        credentials: "include",
  ...
  async function handleRotatePassword() {
    ...
    setRotating(true);
    try {
      const res = await demoFetch(`/api/servers/${identifier}/databases/${viewing.id}/rotate-password`, {
        method: "POST",
        credentials: "include",
      });
      const body = await res.json().catch(() => null);
  ...
  async function handleDelete() {
    if (!deleting) return;
    try {
      const res = await demoFetch(`/api/servers/${identifier}/databases/${deleting.id}`, {
        method: "DELETE",
        credentials: "include",
      });
      if (!res.ok) throw new Error();
