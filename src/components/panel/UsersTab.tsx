...
export function UsersTab({ identifier }: { identifier: string }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await demoFetch(`/api/servers/${identifier}/users`, { credentials: "include" });
  ...
  async function handleInvite() {
    ...
    if (!trimmed) return;
    try {
      const res = await demoFetch(`/api/servers/${identifier}/users`, {
        method: "POST",
        credentials: "include",
  ...
  async function handleRemove() {
    if (!removing) return;
    try {
      const res = await demoFetch(`/api/servers/${identifier}/users/${removing.uuid}`, {
        method: "DELETE",
        credentials: "include",
      });
      if (!res.ok) throw new Error();
