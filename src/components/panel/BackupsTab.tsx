...
export function BackupsTab({ identifier }: { identifier: string }) {
  ...
  const load = useCallback(async () => {
    try {
      const res = await demoFetch(`/api/servers/${identifier}/backups`, { credentials: "include" });
  ...
  async function handleCreate() {
    ...
    try {
      const res = await demoFetch(`/api/servers/${identifier}/backups`, {
        method: "POST",
        credentials: "include",
  ...
  async function handleRestore() {
    if (!restoring) return;
    try {
      const res = await demoFetch(`/api/servers/${identifier}/backups/${restoring.uuid}/restore`, {
        method: "POST",
        credentials: "include",
      });
      if (!res.ok) throw new Error();
  ...
  async function handleDelete() {
    if (!deleting) return;
    try {
      const res = await demoFetch(`/api/servers/${identifier}/backups/${deleting.uuid}`, {
        method: "DELETE",
        credentials: "include",
      });
      if (!res.ok) throw new Error();
  ...
  async function handleDownload(backup: Backup) {
    try {
      const res = await demoFetch(`/api/servers/${identifier}/backups/${backup.uuid}/download`, { credentials: "include" });
      if (!res.ok) throw new Error();
  ...
  async function handleToggleLock(backup: Backup) {
    try {
      const res = await demoFetch(`/api/servers/${identifier}/backups/${backup.uuid}/lock`, {
        method: "POST",
        credentials: "include",
      });
      if (!res.ok) throw new Error();
