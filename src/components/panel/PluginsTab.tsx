...
export function PluginsTab({ identifier, server }: { identifier: string; server: GameServer }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await demoFetch(`/api/servers/${identifier}/plugins`, { credentials: "include" });
  ...
  async function handleInstall(version: PluginVersion) {
    ...
    try {
      const res = await demoFetch(`/api/servers/${identifier}/plugins/install`, {
        method: "POST",
        credentials: "include",
  ...
  async function handleUninstall() {
    ...
    try {
      const res = await demoFetch(`/api/servers/${identifier}/plugins/${uninstalling.id}`, {
        method: "DELETE",
        credentials: "include",
      });
      if (!res.ok) {
  ...
  async function handleDeleteUnmanaged() {
    ...
    try {
      const res = await demoFetch(`/api/servers/${identifier}/plugins/unmanaged`, {
        method: "DELETE",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleUpdate(plugin: InstalledPlugin) {
    setBusyId(plugin.id);
    try {
      const versionsRes = await demoFetch(
      ...
      const res = await demoFetch(`/api/servers/${identifier}/plugins/${plugin.id}/update`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleToggle(plugin: InstalledPlugin) {
    setBusyId(plugin.id);
    try {
      const res = await demoFetch(`/api/servers/${identifier}/plugins/${plugin.id}/toggle`, {
        method: "POST",
        credentials: "include",
      });
      const body = await res.json().catch(() => null);
