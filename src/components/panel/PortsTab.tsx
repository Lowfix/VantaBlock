...
export function PortsTab({ identifier }: { identifier: string }) {
  ...
  const load = useCallback(async () => {
    try {
      const res = await fetch(`/api/servers/${identifier}/network/allocations`, { credentials: "include" });
  ...
  async function handleCreate() {
    try {
      const res = await fetch(`/api/servers/${identifier}/network/allocations`, {
        method: "POST",
        credentials: "include",
      });
      if (!res.ok) {
  ...
  async function handleSetPrimary(allocation: Allocation) {
    try {
      const res = await fetch(`/api/servers/${identifier}/network/allocations/${allocation.id}/primary`, {
        method: "POST",
        credentials: "include",
      });
      if (!res.ok) throw new Error();
  ...
  async function handleDelete() {
    if (!deleting) return;
    try {
      const res = await fetch(`/api/servers/${identifier}/network/allocations/${deleting.id}`, {
        method: "DELETE",
        credentials: "include",
      });
      if (!res.ok) throw new Error();
