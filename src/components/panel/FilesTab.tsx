...
export function FilesTab({ identifier }: { identifier: string }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/servers/${identifier}/files?directory=${encodeURIComponent(path)}`, {
        credentials: "include",
      });
  ...
  async function openEditor(entry: FileEntry) {
    ...
    setEditContent("");
    try {
      const res = await fetch(`/api/servers/${identifier}/files/contents?file=${encodeURIComponent(joinPath(path, entry.name))}`, {
        credentials: "include",
      });
      if (!res.ok) throw new Error();
  ...
  async function handleSaveEdit() {
    ...
    setEditSaving(true);
    try {
      const res = await fetch(`/api/servers/${identifier}/files/contents`, {
        method: "PUT",
        credentials: "include",
  ...
  async function handleRename() {
    ...
    try {
      const res = await fetch(`/api/servers/${identifier}/files/rename`, {
        method: "PUT",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleDelete() {
    if (!deleting) return;
    try {
      const res = await fetch(`/api/servers/${identifier}/files/delete`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleCreateFolder() {
    ...
    try {
      const res = await fetch(`/api/servers/${identifier}/files/folder`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleUploadFile(file: globalThis.File) {
    ...
    try {
      ...
      const res = await fetch(`/api/servers/${identifier}/files/upload`, {
        method: "POST",
        credentials: "include",
        body: formData,
