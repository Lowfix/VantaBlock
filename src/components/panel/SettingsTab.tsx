...
export function SettingsTab({ identifier, server }: { identifier: string; server: GameServer }) {
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const res = await demoFetch(`/api/servers/${identifier}`, { method: "DELETE", credentials: "include" });
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const res = await demoFetch(`/api/servers/${identifier}/settings/rename`, {
        method: "POST",
        credentials: "include",
  ...
  async function handleReinstall() {
    ...
    try {
      const res = await demoFetch(`/api/servers/${identifier}/settings/reinstall`, {
        method: "POST",
        credentials: "include",
      });
      if (!res.ok) throw new Error();
