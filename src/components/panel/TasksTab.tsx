...
export function TasksTab({ identifier }: { identifier: string }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await demoFetch(`/api/servers/${identifier}/schedules`, { credentials: "include" });
  ...
  async function toggleTask(task: ScheduleRow) {
    try {
      const res = await demoFetch(`/api/servers/${identifier}/schedules/${task.id}`, {
        method: "PATCH",
        credentials: "include",
  ...
  async function handleCreate() {
    ...
    try {
      const res = await demoFetch(`/api/servers/${identifier}/schedules`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      await demoFetch(`/api/servers/${identifier}/schedules/${schedule.id}/tasks`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleDelete() {
    if (!deleting) return;
    try {
      const res = await demoFetch(`/api/servers/${identifier}/schedules/${deleting.id}`, {
        method: "DELETE",
        credentials: "include",
      });
      if (!res.ok) throw new Error();
  ...
  async function handleExecute(task: ScheduleRow) {
    try {
      const res = await demoFetch(`/api/servers/${identifier}/schedules/${task.id}/execute`, {
        method: "POST",
        credentials: "include",
      });
      if (!res.ok) throw new Error();
