...
export function ServerPanelPage() {
  ...
  function handlePower(action: "start" | "stop" | "restart" | "kill") {
    ...
    if (isLiveServer && myIdentifier) {
      ...
      fetch(`/api/servers/${myIdentifier}/power`, {
        method: "POST",
        credentials: "include",
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-void">
        <div className="text-center">
          ...
          <button onClick={() => navigate("/dashboard")} className="mt-3 text-accent-400 hover:text-accent-300 text-sm">
            Back to dashboard
          </button>
        </div>
      </div>
      ...
      <div className="min-w-0 flex-1">
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-line-soft bg-void/90 px-4 backdrop-blur-lg sm:px-6">
          ...
          <div className="flex shrink-0 items-center gap-4">
            ...
            <Link to="/dashboard" className="flex items-center gap-1.5 text-[13px] font-medium text-text-lo transition-colors hover:text-bad">
              <LogOut size={14} />
              Logout
            </Link>
          </div>
