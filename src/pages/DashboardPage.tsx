...
export function DashboardPage() {
  ...
  if (currentUser.isOwner) return <OwnerOverviewPage />;
  ...
  function handleAction(id: string, action: "start" | "stop" | "restart" | "kill") {
    ...
    fetch(`/api/servers/${identifier}/power`, {
      method: "POST",
      credentials: "include",
    ...
    <DashboardShell title="Overview">
      <div className="mb-8 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        ...
        <Button size="md" onClick={() => setDeploying(true)}>
          <Plus size={15} /> {currentUser?.isAdmin ? "Deploy new server" : "Request a new server"}
        </Button>
      </div>
