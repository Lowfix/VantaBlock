...
entry further down) so the site can have a second, static page. `src/App.tsx` now wraps
`<Routes>`/`<Route>` around `/` (`LandingPage`, unchanged) and a new `/get-started`
(`GetStartedPage`). Added `public/_redirects` (`/*  /index.html  200`) — required for Cloudflare
Pages to serve `/get-started` on a direct load or refresh rather than 404ing, since it's serving
static files with no knowledge of client-side routes.

Every plan card in `FriendsPhaseNotice.tsx`'s fan (`PlanCardBody`, shared by `PlanFan` and
`MobilePlanList`) now has a "Deploy {plan.name}" button/link, placed once in the JSX right after
the (collapsible) feature checklist — since the checklist collapses to zero height/margin when a
card isn't active, this single placement naturally sits directly below the spec block on collapsed
cards and below the full checklist on the active one, no separate collapsed/expanded branch
needed. Links to `/get-started?plan={plan.id}`.

**Real bug caught before it shipped, not after:** the fan's card wrapper was a `<button>`
(`PlanFan`, click-to-feature). Nesting the new `<Link>` (an `<a>`) inside a `<button>` is invalid
HTML — nested interactive content — and unreliable to click across browsers. Converted the card
wrapper from `<button>` to `<div role="button" tabIndex={0}>` with explicit `onKeyDown` handling
for Enter/Space, keeping `aria-pressed`/`aria-label` as before. Verified with Playwright that
keyboard activation (`.focus()` + `Enter`) still features a card, and that a real pointer click on
a `Deploy` link inside a *non-active* (partially-overlapped) side card still navigates correctly —
the Deploy link's `onClick` also calls `e.stopPropagation()` so clicking it doesn't also fire the
card's own "make this card active" handler on the way out.

**`GetStartedPage.tsx` (new)** — a single mock signup/login page, **not wired to any backend**.
Adapted from the old `LoginPage.tsx`/`RegisterPage.tsx`/`AuthLayout.tsx` (deleted in commit
`584357a`, recovered via `git show 584357a^:...`) rather than built from scratch: same split-card
layout (form left, `VoxelIsland` + highlight list right), same field styling/tokens — with every
real bit stripped out (no `fetch`, no `UserContext`, no Google OAuth button, no `useNavigate` on
success). One component covers both modes via a local `mode` toggle (`"signup"` default,
`"login"`) rather than reintroducing two separate routes, since neither actually needs to exist as
a distinct URL for a mock. Reads `?plan=` via `useSearchParams`, looks it up in `mock-data/plans`,
and — when present — states outright which plan and price the visitor is "signing up for";
gracefully falls back to generic copy with no query param (confirmed via direct nav to
`/get-started` with nothing appended). **On submit, no fake success/logged-in state** — the form
is replaced with an explicit banner: "Real accounts aren't live yet — you're seeing an early
preview of what signing up will look like. Nothing was submitted or saved[, and no {plan} server
was deployed]." Only client-side check performed is a password-match comparison on the signup form
(via `FormData`, not controlled-input state, to avoid re-adding the old pages' per-field
boilerplate for a page that fakes nothing else) — everything else relies on native HTML5
`required`/`type="email"`/`minLength` validation rather than reimplementing the old pages' full
manual validation.

**Verified with a real browser, not just a clean build** — `npx tsc -b --force`/`npm run
build`/`npm run lint` all clean (lint warnings present are pre-existing, in `pterodactyl/theme/*`
and `Button.tsx`, untouched by this change). Then a scratch Playwright install driving `chromium`
against `npm run dev`: clicked a real "Deploy Sapling" card button from the landing page and
confirmed it lands on `/get-started?plan=sapling` showing "You're signing up for the Sapling
plan"; filled in and submitted the signup form and confirmed the honest preview banner appears
(not a fake logged-in state); toggled to login mode and back; navigated directly to
`/get-started?plan=sprout` and to bare `/get-started` (no query param) and confirmed both render
correctly without a client-side router refresh; confirmed the original `/` still renders all five
sections with zero console/page errors across every pass; a 390px-viewport pass confirmed the
mobile stacked list's Deploy buttons work too (had to scope the Playwright locator to the visible
DOM copy specifically, since the hidden desktop fan's copy of the same card also exists off-screen
in the DOM at that width — matches the existing `MobilePlanList`/`PlanFan` dual-render pattern,
not a new issue); re-ran the existing reduced-motion check (`transitionDuration` actually `0s`,
featured card opacity `1` immediately) to confirm the height bump below didn't regress it. Bumped
`PlanFan`'s container from `h-[480px]` to `h-[560px]` to give the new button room without
overflowing/clipping against the "Click a card to feature it" caption below it — checked via
screenshot, not just computed math. Dev server and the scratch Playwright install both torn down
afterward.

**Not pushed** — committed locally only, per this project's established practice of needing the
user's separate explicit confirmation before `git push`.

**See also:** `src/App.tsx`, `src/pages/GetStartedPage.tsx`, `public/_redirects`,
`src/components/landing/FriendsPhaseNotice.tsx` (`PlanCardBody`/`PlanFan`), FRONTEND.md (now stale
on "no router, no second page" — worth a follow-up doc update), the code-side teardown entry
further down (where `react-router-dom` was originally removed and why).
