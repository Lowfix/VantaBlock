...
| "Main Node" | Wings (Pterodactyl node id **2**) | LAN `192.168.1.113` (from `WINGS_NODE_IPS` env) | The only Wings node — a prior node (id 1, this dev machine) was retired 2026-08-16. Runs Wings **natively inside a WSL2 "Ubuntu" distro via systemd**, not Docker (dockerized Wings hit an unfixable Docker-outside-of-Docker bind-mount path mismatch — see `pterodactyl/docker-compose.yml`'s trailing comment and `pterodactyl/wings/Dockerfile`, which is dead/reference-only). **This is a separate physical box, *not* the dev machine** — the WSL2 wording has been misread that way at least once. Verified 2026-08-21: the dev machine sits on a different subnet (`192.168.68.x`, reaching this infra over Tailscale) with its own WSL `Ubuntu` distro *stopped*, while Wings answers on `192.168.1.113:8080` regardless. **No SSH access path to this box is documented anywhere** (not the repo, not `/home/glitch/.ssh`, not `/opt/vantablock/.ssh`), so it is currently the one box that can't be inspected or audited from an agent session — worth fixing. |
